...
param(
    [string]$Date,
    [ValidateSet('daily', 'weekly', 'all')][string]$Mode = 'daily',
    [switch]$Interactive,
    [switch]$CheckOnly,
    [switch]$NoOpen
)
...
try {
    if ($Mode -ne 'daily' -and -not [string]::IsNullOrWhiteSpace($Date)) { throw '기간 지정은 전날 요약 모드에서만 지원합니다.' }
    ...
    Write-Host "Pulse Brief 뉴스 요약 및 배포 | 모드: $Mode | 전날: $Date" -ForegroundColor Cyan
    ...
    if (-not $locked) {
        Write-Host '뉴스 요약 작업이 이미 실행 중입니다. 새 작업을 시작하지 않습니다.' -ForegroundColor Yellow
    } else {
        ...
        if ($Mode -ne 'daily') { $arguments = @((Join-Path $PSScriptRoot 'workflow.cjs'), $Mode) }
        ...
        if ($Mode -ne 'daily') { $resultPath = Join-Path $repoRoot 'data/summary-launcher/last-result.json' }
        $result = Get-Content -LiteralPath $resultPath -Raw -Encoding UTF8 | ConvertFrom-Json
        $results = @($result)
        if ($Mode -ne 'daily') { $results = @($result.results) }
        foreach ($result in $results) {
            $target = $result.date
            switch ($result.status) {
                'existing-server' { Write-Host "$target 요약이 운영 DB에 이미 있습니다. 재생성하거나 덮어쓰지 않습니다." -ForegroundColor Yellow }
                'no-articles' { Write-Host "$target 기사가 없습니다. 빈 요약을 생성하지 않습니다." -ForegroundColor Yellow }
                'published' {
                    Write-Host "$target 요약 배포 완료 | 기사 $($result.articleCount)건" -ForegroundColor Green
                ...
                'checked' { Write-Host "$target 확인 완료 | 로컬 요약: $($result.localExists) | 서버 요약: $($result.serverExists)" }
