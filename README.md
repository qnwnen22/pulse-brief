...
Windows 바탕화면의 **뉴스 요약 및 배포**를 실행하면 전날은 Codex로, 주간은 저장된 일간 요약을 로컬에서 합산하여 생성·검증한 뒤 운영 DB에 자동 반영합니다. 별도 초안 검토나 배포 승인은 없습니다. 뉴스 수집 파이프라인의 예약 요약 호출은 제거했으며, 공개 API는 사이트 접속 시 MongoDB에 저장된 요약만 반환합니다.
