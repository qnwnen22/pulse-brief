...
OpenAI API는 전날/주간 이슈 요약을 보강하는 용도로 사용합니다.

요약 흐름:

1. 서버가 로컬 규칙 기반 요약 초안을 생성
2. OpenAI API 키가 설정되어 있으면 AI 요약 시도
3. AI 응답이 성공하면 `Provider = openai`
4. 실패하거나 API 키가 없으면 로컬 요약 유지
5. 생성된 요약은 MongoDB `summaries` 컬렉션에 저장
