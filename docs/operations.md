...
일상적인 전날 요약은 바탕화면의 **전날 뉴스 요약** 도구로 생성부터 배포까지 실행한다. 날짜 중복 확인, 삽입 전용 저장, 배포 실패 복구 절차는 [바탕화면 요약 도구 안내](manual-summary-launcher.md)를 따른다. 서버의 OpenAI 자동 요약은 계속 중단 상태로 유지한다.

아래 기존 import 도구는 같은 날짜 문서를 **교체**하므로, 명시적으로 기존 요약을 수정·교체하는 별도 작업에만 사용한다. 자동 실행 도구는 이 교체 경로를 호출하지 않는다.
